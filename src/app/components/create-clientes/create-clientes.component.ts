...
export class CreateClientesComponent {
  ...
  agregarCliente() {
    ...
    this.loading = true;
    this._clienteService.agregarClientes(cliente).then(() => {
  ...
  editarCliente(id:string){    
    ...
    this._clienteService.actualizarCliente(id,cliente).then(()=>{
      this.loading = false;
      this.toastr.info('el cliente fue modificado con exito', 'Cliente Registrado', { positionClass: 'toast-top-right' })
      this.router.navigate(['/list-clientes']);
    })
  };  
